	...
	*/
	...
	/*
		[문제점]
			...
			: NULL , "알 수 없는 공간값" 이 있기 때문에 확인이 필요함!
		...
		[풀이]
			1) isset($row['userid']) , 담고 있는 대상의 값에 NULL 이 있는지 없는 확인하기!
			2) NULL이 발견되지 않는다면 ? 뒤에 $row['userid'] 를 반환함
			3) NULL이 발견된다면.. ? 뒤에 'no'를 반환함!

			NULL 이 발견되면 알 수 없는 공간이기 때문에 not null을 처리하는 방법으로 채워야 함!
	*/
